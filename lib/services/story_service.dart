...
class StoryService {
  ...
  Future<List<Map<String, dynamic>>> getStoriesByEmotion(
    ...
    if (user == null) {
      return stories.asMap().entries.map((entry) {
        final story = Map<String, dynamic>.from(entry.value);

        story['is_unlocked'] = entry.key == 0;
    ...
    final completedStoryIds = rewards
        .map((reward) => reward['story_id']?.toString())
        .whereType<String>()
        .toSet();
    ...
    for (int i = 0; i < stories.length; i++) {
      ...
      if (i == 0) {
        ...
        final previousStoryId = previousStory['id']?.toString();

        story['is_unlocked'] =
            previousStoryId != null &&
            completedStoryIds.contains(previousStoryId);
      }
